...
describe 'META6.json', {
  ...
  context 'version', {
    it 'is defined', {
      expect(%meta<version> // '').to.match(/\S/);
    }

    it 'has no wildcard parts', {
      expect(Version.new(%meta<version>).parts.grep('*').elems).to.be(0);
    }
  }
  ...
  context 'authors', {
    ...
    it "uses 'authors' rather than the deprecated 'author'", {
      expect(%meta<author>:exists).to.not.be-truthy;
    }
  }
  ...
  context 'source-url', {
    it 'is defined', {
      expect(%meta<source-url> // '').to.match(/\S/);
    }

    it 'points at a host', {
      expect(%meta<source-url> // '').to.match(/^ \w+ '://' <-[/]>+ /);
    }
  }
  ...
  context 'provides', {
    ...
    context 'declared paths exist on disk', {
      for @provides -> $provided {
        it "{$provided.key} maps to {$provided.value}", {
          expect($provided.value.IO.e).to.be-truthy;
        }
      }
    }

    context 'every lib module is declared', {
      for @lib-rakumod -> $rel {
        it $rel, {
